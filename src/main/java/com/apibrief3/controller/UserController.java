...
public record UserController(UserService userService, JwtService jwtService) {
    ...
    public ResponseEntity<AdressDTO> addAdress(@PathVariable Integer userId, @RequestBody AddAddresstoUserRequest addAvisRequest, BindingResult bindingResult){
        if (bindingResult.hasErrors()) throw new ValidationException(bindingResult.getFieldErrors());
        return ResponseEntity.status(HttpStatus.CREATED).body(userService.addAdressToUser(addAvisRequest, userId));
    }
